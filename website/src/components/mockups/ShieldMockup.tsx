...
export const ShieldMockup = () => {
  ...
  const [trackers, setTrackers] = useState(1482);
  const [ads, setAds] = useState(42);

  useEffect(() => {
    const interval = setInterval(() => {
      setTrackers(prev => prev + Math.floor(Math.random() * 2) + 1);
      setAds(prev => prev + 1);
    }, 2800);
        ...
        <div className="grid grid-cols-4 gap-3 mb-5">
          <motion.div 
            ...
            className={`p-3 rounded-xl border flex flex-col gap-1 transition-all ${
              isDark ? 'bg-emerald-950/20 border-emerald-500/30 shadow-emerald-500/5' : 'bg-emerald-50 border-emerald-200 shadow-xs'
            }`}
          >
            ...
            <motion.div 
              ...
              initial={{ scale: 1.05 }}
              animate={{ scale: 1 }}
              className="text-xl font-black text-emerald-500 font-mono"
            >
          ...
          <motion.div 
            whileHover={{ scale: 1.03 }}
            className={`p-3 rounded-xl border flex flex-col gap-1 transition-all ${
              isDark ? 'bg-blue-950/20 border-blue-500/30 shadow-blue-500/5' : 'bg-blue-50 border-blue-200 shadow-xs'
            }`}
          >
            ...
            <motion.div 
              ...
              initial={{ scale: 1.05 }}
              animate={{ scale: 1 }}
              className="text-xl font-black text-blue-500 font-mono"
            >
              {ads}
            </motion.div>
          </motion.div>
          ...
          <motion.div 
            whileHover={{ scale: 1.03 }}
            className={`p-3 rounded-xl border flex flex-col gap-1 transition-all ${
              isDark ? 'bg-purple-950/20 border-purple-500/30 shadow-purple-500/5' : 'bg-purple-50 border-purple-200 shadow-xs'
            }`}
          >
            ...
            <div className="text-xl font-black text-purple-500 font-mono">64 MB</div>
          </motion.div>

          <motion.div 
            whileHover={{ scale: 1.03 }}
            className={`p-3 rounded-xl border flex flex-col gap-1 transition-all ${
              isDark ? 'bg-amber-950/20 border-amber-500/30 shadow-amber-500/5' : 'bg-amber-50 border-amber-200 shadow-xs'
            }`}
          >
            ...
            <div className="text-xl font-black text-amber-500 font-mono">+340%</div>
          </motion.div>
        </div>

        {/* Blocked Ad Banner Placeholder with Radar Pulse */}
        <motion.div 
          animate={{ borderColor: ['rgba(16,185,129,0.3)', 'rgba(16,185,129,0.7)', 'rgba(16,185,129,0.3)'] }}
          transition={{ duration: 3, repeat: Infinity }}
          className={`w-full py-3.5 px-4 rounded-xl border border-dashed flex items-center justify-center gap-2.5 mb-5 relative overflow-hidden ${
            isDark 
