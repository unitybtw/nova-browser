    ...
    <section className="py-24 relative overflow-hidden">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div
          ...
          className="glass rounded-3xl p-4 sm:p-8 shadow-2xl border border-border/60 overflow-hidden max-w-5xl mx-auto"
        >
          <div className="overflow-x-auto">
            <table className="w-full min-w-[600px]">
              <thead>
                <tr>
                  <th className="text-left py-4 px-6 text-foreground/60 font-semibold text-sm w-1/3">Feature</th>
                  {browsers.map((b) => (
                    <th key={b.key} className={`py-4 px-4 text-center font-bold text-sm rounded-t-2xl ${b.highlight ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-foreground/70'}`}>
                      {b.highlight && (
                        <div className="text-[10px] font-bold text-white/90 mb-0.5 uppercase tracking-wider">
              ...
              <tbody>
                {rowsData.map((row, i) => {
                  ...
                  return (
                    <tr
                      key={i}
                      className={`border-b border-border/40 last:border-0 transition-colors hover:bg-foreground/[0.02] ${i % 2 === 0 ? 'bg-foreground/[0.015]' : ''}`}
                    >
                      <td className="py-4 px-6 text-foreground font-medium text-sm">{featureLabel}</td>
                      {browsers.map((b) => (
                        <td
                          key={b.key}
